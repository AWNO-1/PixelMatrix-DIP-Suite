...
class ManualPDFBuilder:
    ...
    def generate_full_manual(self):
        ...
        members = [
            ("1. أواب النزيلي (Awwab Al-Nuzaili)", "نواة المعالجة المكانية، كاشف كاني الحقيقي، وفلاتر الوسيط"),
            ("2. محمد العواضي (Mohammed Al-Awadhi)", "معمل الحسابات والمزج، تجزئة الألوان، وتشفير البيانات LSB"),
            ("3. مشعل حاجب (Mishaal Hajeb)", "تصميم الواجهة السيبرانية، الفحص البيومتري ICAO، وتوليد التقارير")
        ]
        my = 480
        ...
        self.new_page()
        self.add_chapter_title("11", "الأداة 10: عزل الخلفية الذكي (AI MASK)")
        self.add_paragraph("الأداة العاشرة؛ عزل ذكي فائق الدقة للأشخاص والمنتجات وفصل الخلفية بنقرة زر واحدة:")

        self.add_card_box(
            "أزرار وخيارات العزل الذكي",
            [
                "أزرار اختيار المحرك الذكي: الاختيار بين شبكة U-2-Net العميقة أو خوارزمية GrabCut التكرارية.",
                "منزلق حساسية وعتبة القناع (Matting Threshold): لضبط حساسية العزل على أطراف الشعر والأجسام الدقيقة.",
                "خيار تفعيل تحسين وتنعيم الحدود (Refine Borders): لتنعيم الحواف المعزولة ومنع التشوهات المسننة.",
                "زر عزل الخلفية الفوري (Remove Background): تفريغ الخلفية وجعلها شفافة فورياً.",
                "زر تصدير كقناع شفاف PNG: حفظ العنصر المعزول بخلفية شفافة ألفا بجودة كاملة."
            ],
